...
### 🎫 景点查询
- **在线查询**：使用高德地图v5 POI API查询景点信息
  - 支持按兴趣偏好筛选景点
  - 显示景点人均消费（cost字段）和评分（rating字段）
  - 提供景点门票价格参考信息
- **离线RAG查询**：基于本地向量数据库的景点查询
...
**注意**：
- 首次初始化需要下载embedding模型（约500MB），请确保网络连接正常
- 初始化过程需要5-10分钟，请耐心等待
- 向量数据库文件较大（约2GB），已在`.gitignore`中排除
- 景点数据来源于`jingdian/`目录（352个城市的CSV文件）
