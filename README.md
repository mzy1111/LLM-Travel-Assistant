...
### 🎫 景点查询（RAG技术）
- **RAG向量检索**：基于本地向量数据库的智能景点查询系统
  - 📊 **数据规模**：全国352个城市、33,174个景点的详细信息
  - 🔍 **语义搜索**：支持自然语言查询（如"适合亲子游的地方"、"历史文化景点"）
  - 🎯 **精准匹配**：基于向量相似度的语义理解，而非简单关键词匹配
  - ⚡ **快速响应**：毫秒级查询速度，完全离线运行
  - 🗂️ **向量数据库**：使用Chroma存储景点embeddings
  - 🤖 **Embedding模型**：
    - 优先：`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`（多语言支持）
    - 备选：`shibing624/text2vec-base-chinese`（中文专用）
  - 📍 **城市过滤**：支持指定城市精准定位
  - 📝 **详细信息**：返回景点名称、地址、评分、门票、开放时间、游玩建议等
- **在线查询**：使用高德地图v5 POI API作为补充
  - 支持按兴趣偏好筛选景点
  - 显示景点人均消费和评分
  - 提供实时门票价格参考
...
**RAG景点查询系统**是本项目的核心特性之一，强烈建议初始化：

```bash
# 初始化景点向量数据库（加载全国352个城市、33,174个景点数据）
python scripts/init_attraction_db_simple.py
```

**初始化说明**：
- 📥 **首次运行**：自动下载embedding模型（约500MB），请确保网络连接正常
- ⏱️ **处理时间**：初始化需要5-10分钟，请耐心等待
- 💾 **存储空间**：向量数据库约2GB，已在`.gitignore`中排除
- 📂 **数据来源**：`jingdian/`目录（352个城市的CSV文件）
- 🔄 **持久化**：初始化后数据持久化存储，后续使用无需重新初始化
...
加载 北京 的景点数据：90 个景点
...
   加载了 352 个城市的数据，共 33,174 个景点

2. 创建向量数据库...
正在向量化文档...
向量数据库创建完成，文档数量：33,174
...
- 结合了**信息检索**和**生成式AI**的优势
- 先从知识库检索相关信息，再让LLM基于检索结果生成回答

**系统架构**：
```
用户查询 → 向量化 → 相似度搜索 → 检索Top-K景点 → LLM生成回答
```

**技术实现**：
- 📊 **数据规模**：全国352个城市、33,174个景点的详细信息
- 🗄️ **向量数据库**：Chroma（持久化存储，约2GB）
- 🤖 **Embedding模型**：
  - 优先：`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`（多语言支持）
  - 备选：`shibing624/text2vec-base-chinese`（中文专用）
- 🔍 **检索算法**：基于余弦相似度的向量检索
...
**查询能力**：
- ✅ **语义理解**：
  - "适合亲子游的地方" → 返回适合家庭、儿童的景点
  - "历史文化景点" → 返回博物馆、古迹、宫殿
  - "免费景点" → 返回门票为0元的景点
- ✅ **精准匹配**：基于向量相似度，而非简单关键词匹配
- ✅ **城市过滤**：支持指定城市精准定位
- ✅ **详细信息**：景点名称、地址、评分、门票、开放时间、游玩建议等

**系统优势**：
- ⚡ **快速响应**：毫秒级查询速度
- 🔒 **完全离线**：无需API调用，保护隐私
- 💰 **零成本**：无API调用费用
- 🎯 **高精度**：语义理解能力强，准确理解用户意图
- 🔄 **易扩展**：添加新城市只需添加CSV文件并重新初始化
...
# 2. 自动下载embedding模型（约500MB）
# 3. 构建向量索引（5-10分钟）
# 4. 持久化存储到 data/vector_store/attractions/
```
...
# Agent工具自动调用RAG检索
用户: "北京有什么适合家庭游玩的景点？"
→ RAG检索 → 返回：北京动物园、北京海洋馆、欢乐谷等
→ LLM生成详细推荐

用户: "推荐上海的历史文化景点"
→ RAG检索 → 返回：外滩、豫园、田子坊、朱家角等
→ LLM生成带有历史背景的推荐
```
...
3. **向量数据库**：景点向量数据库文件较大（约2GB），已配置Git忽略，需要自行初始化
4. **景点数据**：`jingdian/`目录包含全国352个城市的景点CSV数据，可用于初始化RAG向量数据库
...
### 2026-02-18 - RAG系统集成与文档完善

#### 🚀 RAG系统集成
- ✅ **景点RAG查询系统**：集成基于向量数据库的智能景点查询
  - 支持全国352个城市、33,174个景点数据
  - 使用Chroma向量数据库存储embeddings
  - 支持语义搜索，理解用户意图
  - 完全离线运行，毫秒级响应
- ✅ **Embedding模型**：
  - 优先使用 `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`
  - 备选 `shibing624/text2vec-base-chinese`
- ✅ **初始化脚本**：提供 `scripts/init_attraction_db_simple.py` 初始化向量数据库
- ✅ **Agent工具集成**：新增 `search_attractions_rag` 和 `get_city_all_attractions_rag` 工具
...
- ✅ **README完善**：
  ...
  - 完善初始化步骤和使用示例
  - 更新技术栈说明
- ✅ **RAG文档**：新增 `RAG_SETUP_GUIDE.md` 和 `RAG_SETUP_SIMPLE.md`

#### 🐛 问题修复
- ✅ **交通路线流式显示**：修复交通路线查询完成后不显示具体内容的问题
  - 后端添加 `show_content: True` 字段
  - 前端处理短内容直接显示，长内容支持折叠
  - 所有内容进行HTML转义和换行格式化
- ✅ **景点费用预算**：景点门票费用正确纳入预算计算
  - 修改 `budget_checker.py` 支持 `actual_costs` 参数
  - 从查询结果中提取实际费用（交通、酒店、景点）
  - 优先使用实际费用，其次使用估算值
